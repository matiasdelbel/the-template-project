...
@HiltViewModel
class CreateWaterViewModel @Inject constructor(
    private val workoutRepository: WorkoutRepository,
) : ViewModel() {

    private var date by mutableStateOf(value = Date())
    ...
    private val duration by derivedStateOf { litersInputFieldState.toTimePeriodOrNull() }
    var litersInputFieldState by mutableStateOf(value = TextFieldValue(text = ""))
        private set
    val durationFieldError by derivedStateOf { duration == null }

    private val averagePace by derivedStateOf { dateInputFieldState.toTimePeriodOrNull() }
    var dateInputFieldState by mutableStateOf(value = TextFieldValue(text = ""))
        private set
    ...
    fun updateDuration(input: TextFieldValue) {
        litersInputFieldState = input.formatAsTimePeriod()
    }

    fun updateAveragePace(input: TextFieldValue) {
        dateInputFieldState = input.formatAsTimePeriod()
    }
    ...
    fun saveWorkout() {
        ...
        viewModelScope.launch {
            workoutRepository.insert(workout = Workout.Running(
                date = date,
